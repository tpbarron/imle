...
for i in 1 2 3 4 5
do
  EXP_PATH="${LOG_DIR}/baseline/ant_dense_rew_x_t1000/${i}/";
  mkdir -p $EXP_PATH
  tmux new-session -s ppo_ant_$i -d "python main.py ${DEFAULT_ARGS} --seed ${i} --log-dir ${EXP_PATH} --env-name AntBulletX-v0"

  EXP_PATH="${LOG_DIR}/vime_baseline/ant_dense_rew_x_t1000/${i}/";
  mkdir -p $EXP_PATH
  ...
  mkdir -p $EXP_PATH
  tmux new-session -s imle_ant_norm_$i -d "python main.py ${DEFAULT_ARGS} --imle --seed ${i} --log-dir ${EXP_PATH} --env-name AntBulletX-v0"

...
LOG_DIR="/media/Backup/trevor1_data/data/outputs/imle/10_26_17/";
# LOG_DIR="test_dir/";
...
do
  ...
  mkdir -p $EXP_PATH
  python main.py ${DEFAULT_ARGS} --imle --seed ${i} --log-dir ${EXP_PATH} --env-name InvertedPendulumSwingupBulletEnv-v0 &

  EXP_PATH="${LOG_DIR}/baseline/inverted_pendulum_swingup_dense_rew_x_t1000/${i}/";
  mkdir -p $EXP_PATH
  python main.py ${DEFAULT_ARGS} --seed ${i} --log-dir ${EXP_PATH} --env-name InvertedPendulumSwingupBulletEnv-v0 &

  EXP_PATH="${LOG_DIR}/vime_baseline/inverted_pendulum_swingup_dense_rew_x_t1000/${i}/";
  mkdir -p $EXP_PATH
  python main.py ${DEFAULT_ARGS} --vime --seed ${i} --log-dir ${EXP_PATH} --env-name InvertedPendulumSwingupBulletEnv-v0 &

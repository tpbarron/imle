...
do
  ...
  do
    running=0;
    if tmux has-session -t imle_hopper_norm_$i;
    then
      running=1;
      sleep 1;
    fi
    if tmux has-session -t imle_hopper_norm_eta_decay_$i;
    then
      running=1;
      sleep 1;
    fi
  ...
  do
    running=0;
    if tmux has-session -t ppo_hopper_$i;
    then
      running=1;
      sleep 1;
    fi
    if tmux has-session -t vime_hopper_$i;
    then
      running=1;
      sleep 1;
    fi

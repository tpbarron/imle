...
DEFAULT_ARGS="--bnn-n-updates-per-step 500 --no-mean-encode --max-episode-steps 500 --num-processes 1 --num-steps 2048 --entropy-coef 0 --ppo-epoch 10 --lr 3e-4 --gamma 0.99 --tau 0.95 --batch-size 64 --num-frames 250000 --use-gae"

# imle comparison
for i in 1 2 3 4 5
do
  ENV_NAME="${LOG_DIR}/baseline/acrobot_continuous_x_tanh/${i}/";
  mkdir -p $ENV_NAME;
  python main.py $DEFAULT_ARGS --seed $i --log-dir $ENV_NAME --env-name "AcrobotContinuousX-v0"

  ENV_NAME="${LOG_DIR}/imle_baseline/acrobot_continuous_x_tanh/${i}/";
  mkdir -p $ENV_NAME;

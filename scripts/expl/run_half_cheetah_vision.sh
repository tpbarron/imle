...
do
  ...
  mkdir -p $EXP_PATH
  python main.py $DEFAULT_ARGS --imle --seed $i --log-dir $EXP_PATH --env-name "HalfCheetahVisionBulletX-v0" --no-cuda
...
do
  ...
  mkdir -p $EXP_PATH
  python main.py $DEFAULT_ARGS --seed $i --log-dir $EXP_PATH --env-name "HalfCheetahVisionBulletX-v0" --no-cuda
  # | tee  "${EXP_PATH}/out.log"
done

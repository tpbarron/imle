...
for i in 1
do
  EXP_PATH="imle_baseline/inverted_pendulum_swingup_etas/${i}/";
  mkdir -p "${LOG_DIR}/${EXP_PATH}";
  python main.py $DEFAULT_ARGS --imle --seed $i --log-dir "${LOG_DIR}/${EXP_PATH}" --env-name "InvertedPendulumSwingupBulletX-v0" --eta 0.1 --no-cuda
done
